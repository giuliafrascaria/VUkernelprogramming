...
void page_init(void)
{
    /* LAB4: Change your code to mark the physical page at MPENTRY_PADDR as
     *       in-use. */
    ...
    size_t pg_boot_end = ((size_t)boot_alloc(0)-KERNBASE) / PGSIZE;
    size_t i;

    cprintf("Physical pages available: %d - %d,  %d - %d\n", 1, npages_basemem,
            pg_boot_end, npages);
    ...
    /* Kernel allocated data starts at EXTPHYSMEM. */

    for (i = pg_boot_end; i < npages; i++) {
        pages[i].pp_ref = 0;
        pages[i].pp_link = page_free_list;
        page_free_list = &pages[i];
    }
